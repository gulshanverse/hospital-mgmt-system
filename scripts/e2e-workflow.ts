...
function bug(msg: string) { fail++; bugs.push(msg); console.error(`  ❌ ${msg}`); }
...
async function runAcceptanceTest() {
  ...
  try {
    const res = await client.auth.login.mutate({ email: 'admin@hms.com', password: 'Password123!' });
  ...
  const doc = docs.find(d => d.specialty?.toLowerCase().includes('cardio')) || docs[0];
  ...
  const apptDate = new Date(Date.now() + 86400000 * randomDays).toISOString().split('T')[0];
  ...
  if (!foundAppt) bug('Appointment not found in getByDate query');
    ...
    else ok(`Appointment departmentName resolved: "${foundAppt.departmentName}"`);
  }
  ...
  const rxResult = await api.prescription.create.mutate({
    patientId: newPat.id,
    items: [
      { medicationName: 'Aspirin 81mg', dosage: '81mg', frequency: 'once daily', duration: '30 days' },
    ],
    notes: 'E2E test prescription',
  });
  if (!rxResult.prescriptionId) bug('prescription.create returned no prescriptionId');
  ...
  if (!tech) bug('No lab_technician user found');
    ...
    await api.lab.assignOrder.mutate({ orderId: labOrder.id, technicianId: tech.id });
  ...
  if (!myOrder) bug('Lab order not found in getOrders');
    ...
    if (myOrder.status !== 'completed') bug(`Lab order status should be completed, got: ${myOrder.status}`);
  ...
  if (availBeds.length === 0) {
    ...
    const inv = await api.billing.createInvoice.mutate({
      ...
      items: [
        { itemType: 'room_charge', description: 'Room charge', unitPrice: 250, quantity: 1 },
        { itemType: 'consultation', description: 'Consultation', unitPrice: 100, quantity: 1 },
        { itemType: 'lab_charge', description: 'Blood panel', unitPrice: 75, quantity: 1 },
      ],
      notes: 'E2E invoice',
    });
    if (!inv.invoiceId) bug('billing.createInvoice returned no invoiceId');
    else ok(`Invoice created: ${inv.invoiceNumber} (invoiceId=${inv.invoiceId})`);

    // Verify invoice details resolve patient name
    const detail = await api.billing.getInvoiceDetails.query({ invoiceId: inv.invoiceId });
    ...
    if (detail.items.length !== 3) bug(`Invoice should have 3 items, got ${detail.items.length}`);
  ...
  if (!kpis) bug('getDashboardKPIs returned null');
    ...
    ok(`KPIs: patients=${kpis.totalPatients}, beds=${kpis.availableBeds}, revenue=$${kpis.totalRevenue}`);
  ...
  for (const rt of roleTests) {
    try {
      const res = await client.auth.login.mutate({ email: rt.email, password: 'Password123!' });
      if (res.user.role !== rt.role) bug(`${rt.email} role mismatch: expected ${rt.role}, got ${res.user.role}`);
